...
# PINN ────────────────────────────────────────────────────────────
class DoublePendulumDampedPINNLoss(PhysicsLoss):
    ...
    def system_params(self, physics):
        ...
        gamma = bounded_param(physics["log_gamma"], *self.bounds["gamma"])
        return m1, m2, l1, l2, g, gamma
    ...
    def acceleration(self, q, qdot, physics):
        m1, m2, l1, l2, g, gamma = self.system_params(physics)
        delta = q[0] - q[1]
        sin_d = jnp.sin(delta)
        cos_d = jnp.cos(delta)
        ...
        f1 = -m2 * l1 * l2 * sin_d * qdot[1] ** 2 - (m1 + m2) * g * l1 * jnp.sin(q[0]) - gamma * qdot[0]
        f2 = m2 * l1 * l2 * sin_d * qdot[0] ** 2 - m2 * g * l2 * jnp.sin(q[1]) - gamma * qdot[1]
