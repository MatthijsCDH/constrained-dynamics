...
def search_space_pinn(trial, base_config):
    ...
    training = dataclasses.replace(
        ...
        lambda_reg      = trial.suggest_float("lambda_reg", 1e-8, 1e-3, log=True),
        use_auto_lambda = False)
    loss = dataclasses.replace(
        base_config.loss,
        lambda_data    = 1.0,
        lambda_physics = trial.suggest_float("lambda_physics", 1e-3, 1, log=True),
        lambda_initial = trial.suggest_float("lambda_initial", 1e-3, 1, log=True),
