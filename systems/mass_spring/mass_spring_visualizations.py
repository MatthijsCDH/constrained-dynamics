...
from systems.mass_spring import data as ms_data
...
class MassSpringVisualizations:
    def __init__(self, config, save_path=None):
        self.config    = config
        self.save_path = save_path
        self.data   = ms_data.make_data(config)
...
def make_live_plot(config, save_path=None):
    return MassSpringVisualizations(config, save_path)
...
class MassSpringAnimation:
    ...
    def animate(self, model, horizon=None, n_frames=None, interval=None, show=True):
        ...
        gs  = gridspec.GridSpec(nrows=3, ncols=1, height_ratios=[1, 1, 2], hspace=0.45)
        ...
        def draw(frame):
            ...
            return (*springs.values(), *masses.values(), marker_true, marker_pred)
...
def make_animation(config, save_path=None):
    return MassSpringAnimation(config, save_path)
