...
from systems.charged_particle import data as cp_data
...
class ChargedParticleVisualizations:
    def __init__(self, config, save_path=None):
        self.config    = config
        self.save_path = save_path
        self.data   = cp_data.make_data(config)
    ...
    def live_learning_plot(self, model, show=True):
        ...
        ax_pred.plot(t, true_states[:, 0], color="blue", label=r"true $x(t)$")
        ax_pred.plot(t, true_states[:, 1], color="teal", label=r"true $y(t)$")
        line_pred_1, = ax_pred.plot([], [], color="red", linestyle="--", linewidth=2, label=r"predicted $x(t)$")
        line_pred_2, = ax_pred.plot([], [], color="orange", linestyle="--", linewidth=2, label=r"predicted $y(t)$")
...
def make_live_plot(config, save_path=None):
    return ChargedParticleVisualizations(config, save_path)


class ChargedParticleAnimation:
    def __init__(self, config, save_path=None):
        self.config    = config
        self.save_path = save_path
        self.data   = cp_data.make_data(config)
    ...
    def animate(self, model, horizon=None, n_frames=None, interval=None, show=True):
        ...
        allxy = np.concatenate([true_states[:, :2], learned_states[:, :2]])
        reach = float(np.max(np.abs(allxy))) * 1.15 + 0.1
        trail = max(1, n_frames // 6)

        fig = plt.figure(figsize=(12, 6))
        gs  = gridspec.GridSpec(nrows=2, ncols=2, width_ratios=[1, 1], height_ratios=[3, 2], hspace=0.35)
        ...
        for ax, label, colour, states in ((ax_true, "true", "tab:blue", true_states),
            ...
            ax.plot(states[:, 0], states[:, 1], color=colour, alpha=0.12, linewidth=1.0, zorder=0)
            ax.plot(0, 0, "+", color="black", markersize=8, zorder=1)
            trails[label], = ax.plot([], [], color=colour, alpha=0.55, linewidth=1.4, zorder=2)
            dots[label],   = ax.plot([], [], "o", color=colour, markersize=9, zorder=3)

        ax_theta.plot(t, true_states[:, 0], color="tab:blue", label="true $x$")
        ax_theta.plot(t, true_states[:, 1], color="teal", label="true $y$")
        ax_theta.plot(t, learned_states[:, 0], color="tab:red", linestyle="--", label="learned $x$")
        ax_theta.plot(t, learned_states[:, 1], color="orange", linestyle="--", label="learned $y$")
        ...
        def draw(frame):
            for label, states in (("true", true_states), ("learned", learned_states)):
                dots[label].set_data([states[frame, 0]], [states[frame, 1]])
                lo = max(0, frame - trail)
                trails[label].set_data(states[lo:frame + 1, 0], states[lo:frame + 1, 1])
            marker.set_data([t[frame]], [true_states[frame, 0]])
            fig.suptitle(f"t = {t[frame]:.2f}   ({t[frame] / self.period:.2f} periods)", fontsize=11)
            return (*dots.values(), *trails.values(), marker)
...
def make_animation(config, save_path=None):
    return ChargedParticleAnimation(config, save_path)
