...
from systems.double_pendulum import data as dp_data
...
class DoublePendulumVisualizations:
...
def make_live_plot(config, save_path=None):
    return DoublePendulumVisualizations(config, save_path)
...
class DoublePendulumAnimation:
    ...
    def animate(self, model, horizon=None, n_frames=None, interval=None, show=True):
        ...
        def draw(frame):
            ...
            return (*rods.values(), *bobs.values(), *trails.values(), marker)
...
def make_animation(config, save_path=None):
    return DoublePendulumAnimation(config, save_path)
